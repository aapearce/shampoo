...
export default function GenerateBrainstorm() {
  ...
  const [mode, setMode] = useState('generate') // 'generate' | 'brainstorm'
  ...
  async function handleGenerate() {
    setLoading(true)
    setStory('')
    const prompt = `Write a complete creative story for a student aged ${ageGroup}.
...
Write an engaging, age-appropriate story with a clear beginning, middle, and end. Use rich literary devices. Length: 400-600 words.`
    ...
    try {
      ...
      setStory(data.content.map(i => i.text || '').join(''))
    } catch { setStory('Failed to generate story. Please try again.') }
    setLoading(false)
  }
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: 'claude-sonnet-4-20250514',
          max_tokens: 1000,
          system: `You are a warm, encouraging creative writing coach for students aged ${ageGroup}. 
Guide them through building their story interactively. Ask one focused question at a time.
Help them develop characters, plot, setting, and literary devices step by step.
After 4-5 exchanges, offer to write a short opening paragraph based on what they've told you.
Keep responses concise and enthusiastic.`,
      ...
      setBrainstormHistory(prev => [...prev, { role: 'assistant', content: 'Sorry, something went wrong. Please try again.' }])
    }
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: 'claude-sonnet-4-20250514',
          max_tokens: 1000,
          system: `You are an encouraging creative writing teacher for students aged ${ageGroup}. Give warm, specific, actionable feedback on their writing. Highlight what works well first, then give 2-3 concrete suggestions for improvement. Offer a rewritten version of their weakest paragraph as an example.`,
          messages: [{ role: 'user', content: `Please give me feedback on this writing and suggest improvements:\n\n${currentDraft}` }]
        })
      })
      const data = await res.json()
      setFeedback(data.content.map(i => i.text || '').join(''))
    } catch { setFeedback('Failed to get feedback. Please try again.') }
  ...
  return (
    <div className="fade-up max-w-4xl">
      ...
      {/* Story options — shared by generate and brainstorm */}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <input value={keywords} onChange={e => setKeywords(e.target.value)}
                placeholder="e.g. moonlight, ancient map, secret..."
                className="w-full bg-g800/50 border border-g800 text-silver font-sans text-xs p-2.5 focus:outline-none focus:border-gold/50 placeholder-g700" />
            </div>
          </div>
        </div>
                ...
                {brainstormHistory.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-lg p-3 font-sans text-sm leading-relaxed ${
                      msg.role === 'user'
                        ? 'bg-oxford/40 text-silver border border-oxford/30'
                        : 'bg-g800/60 text-silver border border-g800'
                    }`}>
                      {msg.content}
                    </div>
                  </div>
      ...
      {mode === 'feedback' && (
        <div>
          <textarea value={currentDraft} onChange={e => setCurrentDraft(e.target.value)}
            placeholder="Paste or write your story here to get feedback and suggestions..."
