    ...
    if (photoFile) {
      ...
      messages = [{
        ...
        content: [
          ...
          { type: 'text', text: buildAssessPrompt('[transcribe from image above]', ageGroup) }
        ]
      }]
    } else {
      messages = [{ role: 'user', content: buildAssessPrompt(text, ageGroup) }]
    }
...
Return ONLY valid JSON, no markdown, no explanation. Use this exact structure:
{
  ...
  "badgeEarned": "device name from this list or null: ${LITERARY_DEVICES.join(', ')}"
      ...
      setErrorMsg(e.message || 'Assessment failed. Please try again.')
    }
    setLoading(false)
  }
  ...
  return (
    <div className="fade-up max-w-4xl">
      ...
      {mode === 'type' ? (
        ...
        <div onClick={() => fileRef.current?.click()}
          ...
          {photoFile
            ...
            : <><span className="text-3xl mb-2">📷</span><p className="font-sans text-sm text-g600">Click to upload a photo of your writing</p><p className="font-sans text-xs text-g700 mt-1">JPG, PNG supported</p></>
          }
      ...
      {errorMsg && (
        <div className="mt-6 border border-red-900/40 p-4 text-red-400 font-sans text-sm">{errorMsg}</div>
      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {(result.devices||[]).map(d => (
                <div key={d.name} className="border border-g800 p-3">
                  ...
                  {d.example && d.example !== 'not found' && <p className="font-sans text-xs text-g600 italic mb-1">"{d.example}"</p>}
            ...
            <div className="border border-g800 p-5 mb-6">
              ...
              <ul className="space-y-1">
                {result.strengths.map((s,i) => (
                  <li key={i} className="font-sans text-sm text-silver flex items-start gap-2"><span className="text-gold mt-0.5">✦</span>{s}</li>
                ))}
              </ul>
            </div>
          )}
                    ...
                    {imp.classicExample && (
                      <div className="bg-g800/40 border border-g800 p-3 mt-2">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {imp.classicLink && <a href={imp.classicLink} target="_blank" rel="noreferrer" className="font-sans text-xs text-oxford hover:text-gold transition-colors">Read more →</a>}
                        </div>
                      </div>
                    )}
                  </div>
