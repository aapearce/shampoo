...
export default function UploadAssess() {
  ...
  return (
    <div className="fade-up max-w-4xl">
      ...
      {errorMsg && <div className="mt-6 p-4 font-sans text-sm" style={{border:'1px solid rgba(220,38,38,0.3)',color:'#f87171'}}>{errorMsg}</div>}

      {result && (
        <div className="mt-10 fade-up">
          {newBadge && (
            <div className="p-4 mb-6 flex items-center gap-4" style={{border:'1px solid #D4AF37',background:'rgba(212,175,55,0.08)'}}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {(result.devices||[]).map(d => (
                <div key={d.name} className="p-3" style={S.border}>
                  ...
                  {d.example&&d.example!=='not found'&&<p className="font-sans text-xs italic mb-1" style={S.body}>"{d.example}"</p>}
                  ...
                  <div key={i} className="pl-4" style={{borderLeft:'2px solid #1F3A5F'}}>
                    ...
                    {imp.classicExample && (
                      <div className="p-3 mt-2" style={{background:'rgba(17,32,64,0.5)',border:'1px solid #1A3358'}}>
                        <p className="font-serif text-xs italic leading-relaxed mb-2" style={{color:'rgba(245,236,215,0.75)'}}>{imp.classicExample}</p>
                        <div className="flex items-center justify-between">
                          <span className="font-sans text-xs" style={{color:'rgba(212,175,55,0.7)'}}>— {imp.classicSource}</span>
                          {imp.classicLink&&<a href={imp.classicLink} target="_blank" rel="noreferrer"
                            className="font-sans text-xs transition-colors" style={{color:'#7A9CC0'}}>Read more →</a>}
                        </div>
                      </div>
                    )}
                  </div>
